...
def SendVideo(userid, msgid):
    ...
    data = {
        "chat_id": userid,
        "caption": "<b>Video Downloaded from</b> @tiktokdownloaderbbot!\n\n<b>EN</b> : <i>if video blank send url again!</i>\n<b>ID</b> : <i>TEEdy yooooo</i>",
        "parse_mode": "html",
...
def Bot(update):
    try:
        ...
        if meseg.startswith('/start'):
            ...
            text = "EN : support this bot by subscribing @hulejob to the developer channel  Thanks\n\nID "
            SendMsg(userid, text, msgid)
            return
    except KeyError:
        return
